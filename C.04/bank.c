...
Bank*Bank_Init(int numBranches, int numAccounts, AccountAmount initalAmount,
          AccountAmount reportingAmount,int numWorkers) {
  ...
  pthread_mutex_init(&bank->bankTransferLock, NULL);
  pthread_mutex_init(&bank->bankFinishDayLock, NULL);
  pthread_mutex_init(&bank->bankFinishedReportSem, NULL);
  pthread_cond_init(&bank->finishedReport, NULL);
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for (unsigned int i = 0; i < bank->numberBranches; i++) {
        pthread_mutex_unlock(&bank->branches[i].branchLock);
      }
      return err;
    }
