...
{
  pthread_mutex_lock(&bank->bankTransferLock);
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&bank->bankTransferLock);
...
int Report_DoReport(Bank *bank, int workerNum) {
  ...
  pthread_mutex_lock(&bank->bankFinishDayLock);
  bank->numWorkerFinished++;

  if(bank->numWorkerFinished == numWorkers) {
    Report *rpt = bank->report;
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
        ...
        bank->numWorkerFinished = 0;
        pthread_cond_broadcast(&bank->finishedReport);
        pthread_mutex_unlock(&bank->bankFinishDayLock);
  ...
  pthread_cond_wait(&bank->finishedReport, &bank->bankFinishDayLock);
  pthread_mutex_unlock(&bank->bankFinishDayLock);

  return bank->report_ERR;
}
