...
(define (intersection-points circles)
  (if (< (length circles) 2) '()
    (append 
      (apply append 
        (map (lambda (otherCircle) 
            (intersect (car circles) otherCircle)
          )
          (cdr circles)
        )
      )
...
(define (distance-product pt lst)
  (if (< (length lst) 1) 1
    (apply *
      (map (lambda (distPt)
        (if (and (= (car pt) (car distPt)) (= (cadr pt) (cadr distPt))) 1
          (dist pt distPt)
        ) ) lst
      ) 
    )
...
(define (rate-points lst)
  (if (< (length lst) 2) 1
    (map (lambda (currPt) 
      (list (distance-product currPt lst) currPt)
    ) lst )
  )
)

(define (point_cmp p1 p2)
  (< (car p1) (car p2))
)

(define (sort-points lst)
  (quicksort lst point_cmp)
)
...
(define (clumped-points lst)
  (if (< (length lst) 2) lst
    (prefix-of-list
      (map (lambda (currPt)
        (cadr currPt)
      ) (sort-points (rate-points lst)) )
      (quotient (length lst) 2)
    )
  )
)

(define (find_avarage lst)
  (list (/ (apply + 
    (map car lst)) (length lst)) 
    (/ (apply + 
    (map cadr lst)) (length lst)) 
  )
)

(define (average-point lst)
  (let ((avgPt (find_avarage lst)))
    (list (distance-product avgPt lst) avgPt)
  )
)

(define (best-estimate lst)
  (avarage-point (clumped-points (intersection-points lst)))
)

(define (where-am-i dist stars)
  (sort-points
    (map (lambda (currLists)
      (best-estimate currLists)
    ) (all-guesses dist stars) ) )
)
