...
static void BuildIndices(const char *feedsFileName, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static void ProcessFeed(const char *remoteDocumentName, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static void PullAllNewsItems(FILE *dataStream, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static void QueryIndices(hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
static void ProcessResponse(const char *word, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl);
...
static const int kStartAlloc = 4;
...
static int StringHash(const void *v, int numBuckets) {
  char* s = *(char**)v;
  int i; 
  unsigned long hashcode = 0;
  for (i = 0; i < strlen(s); i++) 
    hashcode = hashcode * kHashMultiplier + tolower((unsigned char)s[i]);
...
static int AppearHash(const void* ap, int numBuckets) { 
  char **wordPtr = &(((appeared*)ap)->word);
  return StringHash(wordPtr, numBuckets); 
}

void AppearFree(void* appear) {
  appeared* ap = (appeared*)appear;
  if(ap->word != NULL){
    free(ap->word); 
  }
  if (ap->articles != NULL) {
    VectorDispose(ap->articles); 
    free(ap->articles);
  }
}

int AppearCmp(const void* first, const void* second){
  return strcasecmp(((appeared*)first)->word, ((appeared*)second)->word); 
}

void ArticleFree(void* art) {
...
int ArticleCmp(const void* first, const void* second){
  article a = *(article*)first; 
  article b = * (article*)second; 
  if(strcasecmp(a.fullName, b.fullName) == 0 || (strcasecmp(a.name, b.name) == 0 && strcasecmp(a.serverName, b.serverName) == 0)){
    return 0; 
  }
  return strcasecmp(a.fullName, b.fullName); 
}

int ArticleCountCmp(const void* first, const void* second){
  article a = *(article*)first; 
  article b = * (article*)second; 
  return b.count - a.count; 
}

void storeStopWords(hashset* stopWords){
  FILE* stopFile = fopen("data/stop-words.txt","r"); 
  assert(stopFile != NULL); 
  streamtokenizer stopTokens; 
  STNew(&stopTokens, stopFile, kTextDelimiters, true); 
  char token[1024]; 
  while(STNextToken(&stopTokens, token, sizeof(token))){
    char* stopWord = strdup(token); 
    HashSetEnter(stopWords, &stopWord); 
  }
  STDispose(&stopTokens); 
  fclose(stopFile); 
}
...
int main(int argc, char **argv) {
  ...
  HashSetNew(&stopWords, sizeof(char*), nBuckets,StringHash, StringCmp, StringFree);
  HashSetNew(&appeareds, sizeof(appeared), nBuckets, AppearHash, AppearCmp, AppearFree);
  VectorNew(&usedUrl, sizeof(article), ArticleFree, kStartAlloc);
  ...
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopWords, &appeareds, &usedUrl);
  QueryIndices(&stopWords, &appeareds, &usedUrl);
  curl_global_cleanup();

  HashSetDispose(&stopWords);
  HashSetDispose(&appeareds);
  VectorDispose(&usedUrl);
...
static void BuildIndices(const char *feedsFileName, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, stopWords, wordsInArticles, usedUrl);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stopWords, wordsInArticles, usedUrl);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset* stopWords, hashset* wordsInArticles , vector* usedUrl) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stopWords, wordsInArticles, usedUrl);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stopWords, wordsInArticles, usedUrl);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, stopWords, wordsInArticles, usedUrl);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stopWords, wordsInArticles, usedUrl);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  url currUrl;
  URLNewAbsolute(&currUrl, articleURL);

  article currArt; 
  currArt.fullName = strdup(currUrl.fullName);  
  currArt.serverName = strdup(currUrl.serverName);
  currArt.name = strdup(articleTitle);
  currArt.count = 0;
  
  URLDispose(&currUrl);
  if(VectorSearch(usedUrl, &currArt, ArticleCmp, 0, false) != -1){
    ArticleFree(&currArt);
    fclose(tmpDoc);
    return;
  }

  VectorAppend(usedUrl, &currArt);
  STNew(&st, tmpDoc, kTextDelimiters, false);
  ScanArticle(&st, articleTitle, articleDescription, articleURL, stopWords, wordsInArticles, usedUrl);
  STDispose(&st);
  fclose(tmpDoc);
}
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        char* tmp = word;
        if (HashSetLookup(stopWords, &tmp) == NULL) {
          appeared* articlesOfWord; 
          appeared tmpAp;
          tmpAp.word = tmp;
          if(HashSetLookup(wordsInArticles, &tmpAp) == NULL){
            tmpAp.word=strdup(tmp);
            tmpAp.articles = malloc(sizeof(vector));
            VectorNew(tmpAp.articles, sizeof(article), ArticleFree, kStartAlloc);
            HashSetEnter(wordsInArticles, &tmpAp);
            articlesOfWord = &tmpAp;
          } else {
            tmpAp.word = tmp;
            tmpAp.articles = NULL; 
            articlesOfWord = HashSetLookup(wordsInArticles, &tmpAp);
          }
          ...
          url currUrl;
          URLNewAbsolute(&currUrl, articleURL);
          newArticle.fullName = strdup(currUrl.fullName);  
          newArticle.serverName = strdup(currUrl.serverName);
          newArticle.name = strdup(articleTitle);
          URLDispose(&currUrl);

          int idx = VectorSearch(articlesOfWord->articles, &newArticle, ArticleCmp, 0, false);
          if(idx == -1){
            newArticle.count = 1;
            VectorAppend(articlesOfWord->articles, &newArticle);
          } else { 
            article* currArticle = VectorNth(articlesOfWord->articles, idx);
            currArticle->count++;
            ArticleFree(&newArticle);
          }
...
static void QueryIndices(hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopWords, wordsInArticles, usedUrl);
  }
}
...
static void ProcessResponse(const char *word, hashset* stopWords, hashset* wordsInArticles, vector* usedUrl) {
  if (WordIsWellFormed(word)) {
    ...
    if(HashSetLookup(wordsInArticles, &curr) == NULL){
      printf("None of today's news articles contain the word \"%s\".\n", word);
      return; 
    }

    appeared* articlesOfWord = HashSetLookup(wordsInArticles, &curr);

    int numArticles = VectorLength(articlesOfWord->articles);
    if(numArticles > 10){
      numArticles = 10;
    }
    VectorSort(articlesOfWord->articles, ArticleCountCmp);

    for(int i = 0; i < numArticles; i++){
      article* currArt = VectorNth(articlesOfWord->articles, i);
      if(currArt->count == 1){
        printf("%d.) \"%s\" [search term occurs %d time]\n", i + 1, currArt->name, currArt->count);
      } else {
