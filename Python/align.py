...
def findOptimalAlignment(strand1, strand2, memoryze):
    # if one of the two strands is empty, then there is only
    # one possible alignment, and of course it's optimal
    if (strand1, strand2) in memoryze:
        return memoryze[(strand1, strand2)]
    
    positives = ""
    negatives = ""
    if len(strand1) == 0:
        negatives = "2" * len(strand2)
        positives = " " * len(strand2)
        newStr1 = " " * len(strand2)
        newStr2 = strand2
        res = [len(strand2) * -2, positives, negatives, newStr1, newStr2]
        memoryze[(strand1, strand2)] = res
        return res
    if len(strand2) == 0: 
        negatives = "2" * len(strand1)
        positives = " " * len(strand1)
        newStr1 = strand1
        newStr2 = " " * len(strand1)
        res = [len(strand1) * -2, positives, negatives, newStr1, newStr2]
        memoryze[(strand1, strand2)] = res
        return res
    ...
    # they actually match.
    bestWith = findOptimalAlignment(strand1[1:], strand2[1:], memoryze)
    newStr1 = strand1[0] + bestWith[3]
    newStr2 = strand2[0] + bestWith[4]
    if strand1[0] == strand2[0]:
        positives = "1" + bestWith[1]
        negatives = " " + bestWith[2]
        res = [bestWith[0] + 1, positives, negatives, newStr1, newStr2]
        memoryze[(strand1, strand2)] = res
        return res
    # no benefit from making other recursive calls

    best = bestWith[0] - 1
    positives = " " + bestWith[1]
    negatives = "1" + bestWith[2]
    # It's possible that the leading base of strand1 best
    # matches not the leading base of strand2, but the one after it.
    bestWithout = findOptimalAlignment(strand1, strand2[1:], memoryze)
    # penalize for insertion of space
    if (bestWithout[0] - 2) > best:
        best = (bestWithout[0] - 2)
        positives = " " + bestWithout[1]
        negatives = "2" + bestWithout[2]
        newStr1 = " " + bestWithout[3]
        newStr2 = strand2[0] + bestWithout[4]


    # opposite scenario
    bestWithout = findOptimalAlignment(strand1[1:], strand2, memoryze)
    # penalize for insertion of space
    if (bestWithout[0] - 2) > best:
        best = (bestWithout[0] - 2)
        positives = " " + bestWithout[1]
        negatives = "2" + bestWithout[2]
        newStr1 = strand1[0] + bestWithout[3]
        newStr2 = " " + bestWithout[4]

    res = [best, positives, negatives, newStr1, newStr2]
    memoryze[(strand1, strand2)] = res
    return res
...
def printAlignment(seq, out = sys.stdout):
    out.write("                Score: " + str(seq[0]) + "\n")
    out.write("                + " + seq[1] + "\n")
    out.write("                  " + seq[3] + "\n")
    out.write("                  " + seq[4] + "\n") 
    out.write("                - " + seq[2] + "\n")
...
def main():
    ...
    while (True):
        ...
        strand2 = generateRandomDNAStrand(7, 14)
        sys.stdout.write("Aligning these two strands: " + strand1 + "\n")
        sys.stdout.write("                            " + strand2 + "\n")
        memoryze = {}
        alignment = findOptimalAlignment(strand1, strand2, memoryze)
